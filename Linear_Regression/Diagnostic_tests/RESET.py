...
class RESET():
    ...
    def test(self, fitted_model):
        ...
        model.fit(X=X, y=fitted_model.y, features_names=features_names, target_name=fitted_model.target_name, diagnostic_test=True)
        ...
        self.p_value = np.round(model.calculate_p_value_F_test(F_test=self.F_test, dfn=self.power-1, dfd=len(X)-len(features_names)), 5)