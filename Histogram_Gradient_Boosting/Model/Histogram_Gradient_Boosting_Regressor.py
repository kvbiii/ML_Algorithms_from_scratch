...
class Histogram_Gradient_Boosting_Regressor():
    def __init__(self, n_estimators=100, learning_rate=0.1, loss="squared_error", max_depth=None, max_bins=255, categorical_features=None, random_state=17):
        ...
        self.check_max_bins(max_bins=self.max_bins)
        self.categorical_features = categorical_features
        self.check_categorical_features(categorical_features=self.categorical_features)
    ...
    def check_max_bins(self, max_bins):
        if not isinstance(max_bins, int):
            raise TypeError('Wrong type of max_bins. It should be int.')
    
    def check_categorical_features(self, categorical_features):
        if not isinstance(categorical_features, list) and not isinstance(categorical_features, np.ndarray) and categorical_features != None:
            raise TypeError('Wrong type of categotical_features. It should be list or numpy array.')
        return categorical_features
    
    def fit(self, X, y):
        self.X_train = self.check_X(X=X, train=True)
        self.X_train = self.check_for_object_columns(X=self.X_train)
        list_of_missing_values = self.check_for_missing_values(X=self.X_train)
        self.X_train = self.convert_to_bins(X=self.X_train, list_of_missing_values=list_of_missing_values)
    ...
    def check_X(self, X, train):
        ...
        if(self.fit_used == False):
            if(self.categorical_features != None):
                if(type(self.categorical_features[0]) == str and isinstance(X, pd.DataFrame) == True):
                    self.categorical_features = [index for index, feature_name in enumerate(list(X.columns)) if feature_name in self.categorical_features]
                else:
                    raise TypeError('Provided categorical features could not be found in passed X which does not contain any feature names.')
            else:
    ...
    def check_for_missing_values(self, X):
        list_of_missing_values = []
        for feature in range(0, X.shape[1]):
            list_of_missing_values.append(np.where(np.isnan(X[:,feature]))[0])
        return list_of_missing_values

    def convert_to_bins(self, X, list_of_missing_values):
        if(self.max_bins > X.shape[0]):
            number_of_bins = X.shape[0]
        else:
            number_of_bins = self.max_bins
        all_features = [i for i in range(0, X.shape[1])]
        continous_features = np.setdiff1d(all_features, self.categorical_features)
        X_final = np.zeros(shape=(X.shape[0], X.shape[1]))
        for feature in range(0, X.shape[1]):
            X_without_missing = X[:,feature][~np.isnan(X[:,feature])]
            if(feature in continous_features):
                bins = np.array([np.linspace(np.min(X_without_missing), np.max(X_without_missing), number_of_bins)]).T
                bundled_continous_features = np.array([np.digitize(X_without_missing, bins.squeeze())]).squeeze()
                unique_values, counts = np.unique(bundled_continous_features, return_counts=True)
                for indice in list_of_missing_values[feature]:
                    bundled_continous_features = np.insert(bundled_continous_features, indice, unique_values[counts == counts.max()][0])
                X_final[:, feature] = bundled_continous_features
            else:
                unique_values, counts = np.unique(X_without_missing, return_counts=True)
                for indice in list_of_missing_values[feature]:
                    X_without_missing = np.insert(X_without_missing, indice, unique_values[counts == counts.max()][0])
    ...
    def predict(self, X):
        ...
        list_of_missing_values = self.check_for_missing_values(X=X)
        X = self.convert_to_bins(X=X, list_of_missing_values=list_of_missing_values)
    ...
    def check_fit(self, fit_used):
        if fit_used == False:
            raise AttributeError('Histogram_Gradient_Boosting_Regressor has to be fitted first.')