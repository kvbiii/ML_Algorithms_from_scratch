...
class Lasso_Regressor():
    ...
    def calculate_derivative_of_loss(self, X, y, y_predict, alpha):
        return (-2/X.shape[0]*(np.matmul(X.T, y-y_predict)-alpha/2*np.sum(self.coef_/np.abs(self.coef_)))).reshape(-1,1)

    def calculate_loss(self, X, y, y_predict):
        return 1/X.shape[0]*(np.sum(y-y_predict)**2+self.alpha*np.sum(np.abs(self.coef_)))
...
class Lasso_Classifier():
    ...
    def calculate_derivative_of_loss(self, X, y, coef, y_predict, alpha):
        return (-2/X.shape[0]*(np.matmul(X.T, y-y_predict)-alpha/2*np.sum(self.coef_/np.abs(self.coef_)))).reshape(-1,1)

    def calculate_loss(self, X, y, y_predict, coef):
        return 1/X.shape[0]*(np.sum(y-y_predict)**2+self.alpha*np.sum(np.abs(coef)))
