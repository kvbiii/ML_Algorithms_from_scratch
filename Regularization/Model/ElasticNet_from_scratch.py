...
class ElasticNet_Regressor():
    def __init__(self, l1_term=1, l2_term=1, fit_intercept=True, learning_rate=0.1, max_iter=500, random_state=17):
    ...
    def find_coefficients_gradient_descent(self, X, y, learning_rate, max_iter):
        ...
        for epoch in range(0, max_iter):
            self.losses[epoch] = self.calculate_loss(X=X, y=y, coefs=self.coef_, l1_term=self.l1_term, l2_term=self.l2_term)
            grad_coef = self.calculate_derivative_of_loss(X=X, y=y, coefs=self.coef_, l1_term=self.l1_term, l2_term=self.l2_term)
            self.coef_ = self.coef_ - learning_rate*grad_coef
    
    def calculate_loss(self, X, y, coefs, l1_term, l2_term):
        return 1/X.shape[0]*(np.sum(y-np.matmul(coefs, X.T))**2+l1_term*np.sum(np.abs(coefs))+l2_term*np.sum(coefs**2))
    
    def calculate_derivative_of_loss(self, X, y, coefs, l1_term, l2_term):
        return -2/X.shape[0]*(np.matmul(X.T, y-np.matmul(coefs, X.T))-l1_term/2*coefs/np.abs(coefs)-l2_term*coefs)
...
class ElasticNet_Classifier():
    ...
    def find_coefficients_gradient_descent(self, X, y, learning_rate, max_iter):
        ...
        for klasa in range(0, self.number_of_classes):
            ...
            for epoch in range(0, max_iter):
                self.losses[epoch, klasa] = self.calculate_loss(X=X, y=y[:, klasa], coefs=current_coef, l1_term=self.l1_term, l2_term=self.l2_term)
                grad_coef = self.calculate_derivative_of_loss(X=X, y=y[:, klasa], coefs=current_coef, l1_term=self.l1_term, l2_term=self.l2_term)
