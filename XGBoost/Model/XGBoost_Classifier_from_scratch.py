    ...
    def build_xgboost(self, X, y):
        ...
        for klasa in range(1, len(self.original_classes)):
            ...
            for t in range(0, self.n_estimators):
                Tree = Extreme_Tree_Classifier(X=X, y=y[:,klasa], predictions=predictions, feature_importances_=self.feature_importances_, reg_lambda=self.reg_lambda, gamma=self.gamma, max_depth=self.max_depth, colsample_bytree=self.colsample_bytree, min_child_weight=self.min_child_weight)
...
class Extreme_Tree_Classifier():
    ...
    def weighted_quantile_sketch(self, X, predictions):
        split_points = []
        hessian_ = 1/(1+np.exp(-predictions))*1/(1+np.exp(predictions))
    ...
    def calculate_simmilarity(self, y_in_leaf, predictions_in_leaf, reg_lambda):
        return 1/2*((np.sum(-(y_in_leaf-1/(1+np.exp(-predictions_in_leaf)))))**2)/(np.sum(1/(1+np.exp(-predictions_in_leaf))*1/(1+np.exp(predictions_in_leaf)))+reg_lambda)
    
    def calculate_cover(self, predictions_in_leaf):
        return np.sum(1/(1+np.exp(-predictions_in_leaf))*1/(1+np.exp(predictions_in_leaf)))
...
class Leaf:
    def __init__(self, y, predictions, reg_lambda):
        self.prediction = np.sum(y-1/(1+np.exp(-predictions)))/(np.sum(1/(1+np.exp(-predictions))*1/(1+np.exp(predictions)))+reg_lambda)
