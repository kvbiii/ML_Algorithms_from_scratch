...
class Decision_Tree_Classifier():
    ...
    def predict(self, X):
        ...
        for row in range(0, X.shape[0]):
            predictions.append(self.get_score(row=X[row,:], node=self.tree, problem="prediction"))
    ...
    def predict_proba(self, X):
        ...
        probabilities = []
        for row in range(0, X.shape[0]):
            probabilities.append(self.get_score(row=X[row,:], node=self.tree, problem="probability"))
...
class Leaf:
    def __init__(self, y, original_classes):
        self.prediction = Decision_Tree_Classifier().most_frequent_class(y=y)
        self.probabilities = Decision_Tree_Classifier().get_proba(y=y, original_classes=original_classes)
