...
class Decision_Tree_Regressor():
    def __init__(self, criterion="squared_error", max_depth=None, min_samples_split=2, max_features=None, random_state=17):
    ...
    def check_X(self, X, train):
        ...
        if(train == False):
            if(self.X_train.shape[1] != X.shape[1]):
                raise ValueError(f"X has {X.shape[1]} features, but Decision_Tree_Regressor is expecting {self.X_train.shape[1]} features as input.")
        return X
    ...
    def predict(self, X):
        ...
        for row in range(0, X.shape[0]):
            predictions.append(self.get_score(row=X[row,:], node=self.tree))
...
class Leaf:
    def __init__(self, y):
        self.prediction = Decision_Tree_Regressor().mean_of_target_in_node(y=y)
